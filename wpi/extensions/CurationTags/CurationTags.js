var CurationTags = window.CurationTags = {};
...
CurationTags.loadAvailableTags = function() {
	...
	$.get(
		mw.util.wikiScript(), {
		action: 'ajax',
			rs: 'CurationTagsAjax::getAvailableTags'
		},
		function( obj , status, answer) {			
			CurationTags.loadAvailableTagsCallback(answer);
		}
	);
...
CurationTags.createToolDivContent = function() {
	var helpTag = "<TD><A title='Show help page' " +
		"href='" + mw.config.values.helpLink + "' target='_blank'>" +
...
CurationTags.applyNewTag = function() {
	...
	if(tagDef && tagDef.revision) {
		tagRev = mw.config.values.pageRevision;
	}
...
CurationTags.removeTag = function(tagName) {
	...
	$.get(
	  mw.util.wikiScript(), {
		action: 'ajax',
		rsargs: [tagName, CurationTags.pageId],
		rs: 'CurationTagsAjax::removeTag'
	  },
	  function( obj , status, answer) {
			CurationTags.removeTagCallback(answer);
          }
	);
/*
	sajax_do_call(
...
CurationTags.removeTagFromPathway = function( tagName, pathway, rowId ) {
	...
	/*improve by checking if there was an answer*/
...
CurationTags.updateTag = function(tagName) {
	...
	if(tagData) {
		CurationTags.saveTag(tagName, tagData.text, mw.config.values.pageRevision);
	}
};
...
CurationTags.saveTag = function(tagName, tagText, tagRev) {
        $.get(
          mw.util.wikiScript(), {
                action: 'ajax',
                rs: "CurationTagsAjax::saveTag",
                rsargs: [tagName, CurationTags.pageId, tagText, tagRev]
          },
	        function( obj , status, answer) {
...
CurationTags.showHistory = function() {
	...
	mw.util.wikiScript(), {
                action: 'ajax',
                rs: "CurationTagsAjax::getTagHistory",
                rsargs: [CurationTags.pageId, "0"]
        },
                function( obj , status, answer) {
			 CurationTags.loadHistoryCallback(answer);
                }
        );
...
CurationTags.loadTags = function() {
	...
	$.get(
	  mw.util.wikiScript(), {
		action: 'ajax',
		rs: 'CurationTagsAjax::getTags',
		rsargs: [CurationTags.pageId, mw.config.values.pageRevision]		
	  },
	  function( obj , status, answer) {
		//alert(obj.responseText);
		//console.log("my object: %o", obj.documentElement.nodeName);
		//console.log(obj.);
		//alert("obj " + obj.documentElement.nodeName);
		CurationTags.loadTagsCallback(answer);
	  }
	);
	...
	/*sajax_do_call(
		...
		[CurationTags.pageId, mw.config.values.pageRevision],
...
CurationTags.loadTag = function(tagName) {
	$.get(
          mw.util.wikiScript(), {
                action: 'ajax',
                rs: 'CurationTagsAjax::getTagData',
                rsargs: [tagName, CurationTags.pageId, mw.config.values.pageRevision]
          },
          function( obj , status, answer) {
               CurationTags.loadTagCallback(answer);
          }
	);
	...
	/*sajax_do_call(
		...
		[tagName, CurationTags.pageId, mw.config.values.pageRevision],
...
CurationTags.checkResponse = function(xhr) {
	//improve this function since now it allows everything
	//return true;
